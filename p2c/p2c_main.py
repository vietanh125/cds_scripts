...
libc.get_point_bridge.argtypes = [array_2d_int, c_float, POINTER(c_int), POINTER(c_int), c_int, c_int]
...
def get_point_bridge(img, roi, flag, left_restriction, right_restriction):
    # libc.get_point(a, 0.6, p_x, p_y, 0, 0, 319)
    # x = c_int(0)
    # y = c_int(0)
    p_x = pointer(c_int(0))
    p_y = pointer(c_int(0))
    libc.get_point_bridge(img, roi, p_x, p_y, flag, left_restriction, right_restriction)
    return p_y.contents.value, p_x.contents.value
...
def set_time_threshold(time_threshold):
    libc.set_time_threshold(time_threshold)
