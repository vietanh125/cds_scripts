...
def check_obstacle(img, threshold):
    ...
    if i_left >= i_right:
        i += range
        while np.sum(img[lower_y:upper_y, i_left - range:i_left])/(max_value * size) <= threshold and i_left - range >= 0:
            i_left -= range
        while np.sum(img[lower_y:upper_y, i_right: i_right + range])/(max_value * size) <= threshold and i_right + range <= IMG_W:
            i_right += range
