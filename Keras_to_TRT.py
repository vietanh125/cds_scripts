...
def TF_to_TRT():
    ...
    with gfile.FastGFile("./tensorRT/TensorRT_12345_" + PRECISION + ".pb", 'wb') as f:
...
def test(n_time_inference=50):
    ...
    with graph.as_default():
        with tf.Session(config=tf.ConfigProto(gpu_options=tf.GPUOptions(per_process_gpu_memory_fraction=0.5))) as sess:
            trt_graph = read_pb_graph('./tensorRT/TensorRT_12345_'+ PRECISION + '.pb')
...
pipe_line("model-mobilenet-iter1234add5-pretrain-bdd")
